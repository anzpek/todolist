...
const ProjectTemplateManager = ({ isOpen, onClose, onSelectTemplate, mode = 'modal' }: ProjectTemplateManagerProps) => {
  ...
  const Content = (
    <div className={`bg-white dark:bg-gray-800 ${mode === 'modal' ? 'rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden' : 'h-full flex flex-col'}`}>
      {mode === 'modal' && (
      ...
      <div className={`p-6 overflow-y-auto ${mode === 'modal' ? 'max-h-[calc(90vh-120px)]' : 'flex-1 custom-scrollbar'}`}>
        <div className="mb-6">
          <button
