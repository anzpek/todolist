import { Clock, Calendar, AlertCircle, CheckCircle2, Circle, ChevronDown, ChevronRight, Repeat, Timer, Edit, Trash2, RotateCcw, ArrowRightFromLine, ArrowRightToLine } from 'lucide-react'
    ...
    <div className={`card ${todo.completed ? 'p-2 mb-1.5 bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800' : 'p-2 mb-1.5'} ${isTaskOverdue && !todo.completed ? 'border-red-300 bg-red-50 dark:bg-red-900/20' : ''}`}>
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* 두 번째 줄: 날짜 정보와 시간 표시 */}
          {(todo.dueDate || todo.completed || todo.startTime) && (
            <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 space-y-0.5">
              {/* 시간 정보 표시 */}
              {(todo.startTime || (todo.dueDate && todo.dueTime)) && (
                <div>
                  {todo.startTime && (
          ...
          {/* 날짜 정보 표시 (시작일/마감일 아이콘화) */}
          <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 space-y-0.5">
            {/* 시작일만 있는 경우 */}
            {todo.startDate && !todo.dueDate && (
              <div className="flex items-center gap-1 text-blue-600 dark:text-blue-400">
                <ArrowRightFromLine className="w-3 h-3" />
                <span>{formatCompactDate(todo.startDate)}</span>
              </div>
            )}

            {/* 마감일만 있는 경우 */}
            {!todo.startDate && todo.dueDate && (
              <div className={`flex items-center gap-1 ${isTaskOverdue && !todo.completed ? 'text-red-600 font-bold' : 'text-gray-600 dark:text-gray-400'}`}>
                <ArrowRightToLine className="w-3 h-3" />
                <span>{formatCompactDate(todo.dueDate)}</span>
