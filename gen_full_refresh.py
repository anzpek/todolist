...
for week in range(5):
    ...
    for day in range(7):
        cell_idx = week * 7 + day
        tasks = '\n            '.join([f'<TextView android:id="@+id/task_{cell_idx}_{t}" android:layout_width="match_parent" android:layout_height="wrap_content" android:textColor="#FFFFFF" android:textSize="10sp" android:maxLines="1" android:ellipsize="end" android:visibility="gone"/>' for t in range(8)])
...
with open(r'c:\Users\dan\Documents\todolist\app\android\app\src\main\res\layout\widget_full_calendar_layout.xml', 'w', encoding='utf-8') as f:
    f.write(content)
print('Full Calendar: 8 tasks per cell with refresh button - DONE')
